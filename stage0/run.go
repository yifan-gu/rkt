...
func addHosts(hostname, rootfs string) {
	var buffer bytes.Buffer
	buffer.WriteString("# Generated by rkt\n\n")
	...
	if err := ioutil.WriteFile(filepath.Join(rootfs, "etc/rkt-hosts"), buffer.Bytes(), 0644); err != nil {
		log.Fatalf("error writing /etc/rkt-hosts: %v\n", err)
	}
...
func Run(cfg RunConfig, dir string, dataDir string) {
	...
	if cfg.Hostname != "" {
		addHosts(cfg.Hostname, destRootfs)
	}
